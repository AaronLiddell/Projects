...
# Fitting Gaussians to Cs137 20 degree 300s spectrum
# NOTE: Generative AI was used to assist in writing the following code.
...
# Function to parse ROI values from the .Spe file (no chopping)
def read_spe_rois(filename):
	roi_list = []
	with open(filename, 'r') as f:
		lines = f.readlines()
				...
				try:
					...
					for roi_line in roi_values:
						...
						if len(parts) == 2:
							...
							roi_list.append((start, end))
				except Exception as e:
					print(f"Error parsing ROI: {e}")
				break
	return roi_list

# Parse ROIs from the .Spe file (no chopping)
roi_list = read_spe_rois(spe_file)
...
plt.ylabel('Counts')
plt.title('Cs137 20° 300s Spectrum with Gaussian Fit(s)')
plt.legend()
plt.show()
